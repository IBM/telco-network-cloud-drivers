...
| startTime        | Time the process started expressed in ISO 1806 format. This time is taken from the server on which LM is running. If the process is yet to start then this property will not be present |
| endTime          | Time the process finished expressed in ISO 1806 format. This time is taken from the server on which LM is running. If the process has not completed or been cancelled, then this property will not be present |
