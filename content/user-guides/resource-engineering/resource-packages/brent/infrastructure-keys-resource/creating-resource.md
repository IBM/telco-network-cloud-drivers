...
private-properties:
  server2_ssh_key:
    description: An SSH key pair created by the HEAT template, used by apache_server2 server. This will be stored in LM (both the public and private key portions, the private key portion will be stored securely).
    type: key
infrastructure:
  Openstack: {}
lifecycle:
...
* `server1_ssh_key`: the name of an existing SSH key (created in Openstack and stored as a shared infrastructure key in LM), used when provisioning apache_server1
